...
public class CaptureTheWool extends JavaPlugin {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(args[0].equals("create") && args.length > 1){
            ...
            if(file.exists()) {
                sendMessage(sender, "file.exists");
                return true;
            }
            try {
        ...
        if(args[0].equals("woolpoint") && args.length > 3) {
            File configFile = new File(new File(getDataFolder(), "games"), args[1]);
            if(!configFile.exists()){
                sendMessage(sender, "file.not_exists");
                return true;
            }
    ...
    public void sendMessage(CommandSender sender, String key){
        if(sender instanceof Player){
            sender.sendMessage(LanguageUtils.getMessage(this, Language.fromPlayer((Player) sender), key, new Language(Locale.SIMPLIFIED_CHINESE)));
        }
    }
